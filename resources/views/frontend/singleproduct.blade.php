...
<div id="custCarousel" class="carousel slide" data-bs-ride="carousel">
    ...
    <div class="carousel-inner">
        ...
        <div class="carousel-item @if($x == 0) active @endif ">
          <div class="show" href="{{asset($image->getUrl())}}">
           <img src="{{asset($image->getUrl())}}" class="d-block w-100" id="show-img"> 
         </div>
       </div>
        <?php $x++;?>
        @endforeach
    ...
    <ol class="carousel-indicators list-inline">
        <?php $x=0;?>
        @foreach ($product->getMedia('products') as $image) 
        <li class="list-inline-item  @if($x == 0) active @endif"> 
          <a id="carousel-selector-0" class="selected" data-bs-slide-to="{{$x}}" data-bs-target="#custCarousel">
    ...
    <div class="col-sm-6">
        <h1 style="font-weight: 400; font-size:2rem;">{{$product->name}}</h1>
        <div class="row mb-3">
            <div class="col-sm-6" style=" padding: 0px;" >
        @if($product->quantity >0)
        <div class="badge bg-warning">In stock: {{$product->quantity}}</div><br>
        @else
        ...
        <h4 style="font-weight: 900;"><small>Kes. </small>{{number_format($product->sale_price,2)}}</h4>
        <h6 style="text-decoration: line-through;"><small>Kes. </small>{{number_format($product->price,2)}}</h6>
        @else
        <h4 style="font-weight: 900;" ><small>Kes. </small>{{number_format($product->price,2)}}</h4>
        @endif
        <div class="row">
          <div class="col-sm-12">
        ...
        <div class="row">
            <div class="col-sm-12">
                @if(isset($product->fuel))<i class="fa fa-plus"></i> Fuel: {{$product->fuel}}<br>@endif
                @if(isset($product->year))<i class="fa fa-plus"></i> Year: {{$product->year}}<br>@endif
                @if(isset($product->engine))<i class="fa fa-plus"></i> Engine size: {{$product->engine}}@endif</div>
        </div> 
    ...
    <div class="row g-2 mb-3 owl-carousel owl-theme">
      ...
      <x-product-thumbnail :product="$similarProduct" class="item"></x-product-thumbnail>
      @endforeach
    </div>
